// "Add support for other database functions
// "Start with 'find'
...
void find(struct Connection *conn, char *findstring) {
	...
	for (i = 0; i < conn->db->num_rows; i++) {
		...
		if (addr->set) {
			if (!strcmp(addr->name, findstring)) {
				hits++;
				Address_print(addr);
			} else if (!strcmp(addr->email, findstring)) {
				hits++;
				Address_print(addr);
			}
		}
...
int main(int argc, char *argv[]) {
	...
	switch (action) {
		...
		case 'f':
			if (argc != 4)
				die("Need a string to find", conn);
